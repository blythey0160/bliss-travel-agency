...
?>
<div id="wte-crumbs">
	<?php do_action( 'wp_travel_engine_breadcrumb_holder' ); // phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound ?>
</div>
<div
	id="wp-travel-trip-wrapper"
	class="wte-trip-content-area"
	itemscope
	itemtype="http://schema.org/LocalBusiness">
	<div class="wte-travel-inner-wrapper">
		<div class="wp-travel-engine-archive-outer-wrap">
			<?php
				...
				$termchildren = get_terms(
					$taxonomy,
					array(
						'orderby' => apply_filters( "wpte_{$taxonomy}_terms_order_by", 'date' ),// phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound
						'order'   => apply_filters( "wpte_{$taxonomy}_terms_order", 'ASC' ),// phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound
					)
				);
				$terms_by_ids = array();
				...
				if ( ! empty( $terms_by_ids ) ) {
					...
					<div class="wte-page-header travescape-wte-page-header" style="background-image: url('<?php echo esc_url($image_url); ?>');">
						<div class="container">
								<div class="row">													
								<?php
								$display_title = apply_filters( 'wpte_display_taxonomy_page_title', true );// phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound
								if ( $display_title ) :
									?>
					...
					<div class="row">	
								<div class="page-content">								
										<?php
										$content = apply_filters( 'the_content', $post->post_content );// phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound
								...
								<div class="item wpte-trip-category">
									...
									<div class="wpte-trip-category-img-wrap">
										<figure class="thumbnail">
											...
											<?php
														$term_object->thumbnail && print( \wp_get_attachment_image(
															...
															apply_filters( 'wp_travel_engine_activities_img_size', 'activities-thumb-size' ),// phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound
															false,
										...
										<div class="wpte-trip-category-overlay">
											...
											<div class="wpte-trip-category-btn">
												<?php printf( '<a href="%1$s" class="wpte-trip-cat-btn">%2$s</a>', esc_url( $term_object->link ), esc_html( 'View All', 'travelscape' ) ); ?>
											</div>
										</div>
									</div>
									<div class="wpte-trip-category-text-wrap">
										<h2 class="wpte-trip-category-title" itemprop="name">
											<a href="<?php echo esc_url( $term_object->link ); ?>">
												<?php echo esc_html( $term_object->name ); ?></a><span class="trip-count"><?php printf( _n( '(%d Trip)', '(%d Trips)', (int) $term_object->count, 'travelscape' ), (int) $term_object->count ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></span>
										</h2>
									</div>
								</div>
								<?php
